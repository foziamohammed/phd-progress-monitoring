...
exports.getInstructorRequests = async (req, res) => {
    ...
    try {
      const requests = await MeetingRequest.find({ instructorId });
      if (!requests.length) {
        return res.status(404).json({ message: 'No requests found for this instructor.' });
      }
      res.status(200).json(requests); // Always return an array
    } catch (error) {
